...
int main(int argc, char *argv[]){
    ...
    for(inst = 0; inst < n_instancias; inst++){
        ...
        busca_local_rotacao(d, d->prioridade, &res);
        clock_t fim = clock();
...
void exibir_tabela_comparativa(Instancia inst[], int n){
    ...
    printf("| %-5s | %-8s | %-6s | %-6s | %-9s | %-9s | %-8s| %-8s| %-12s| %-12s|\n", "Inst.", "Chapa", "NCortesRef.", "NCortesNosso", "Area cm2", "Usada cm2", "Sobra%Ref", "Sobra%Nos", "Tempo Ref(s)", "Tempo Nos(s)");
    printf("+-------+----------+-------------+--------------+-----------+-----------+----------+----------+-------------+-------------+\n");
    ...
    for(int i = 0; i < n; i++){
        ...
        for(int i = 0; i < n; i++){
            ...
            Instancia *d = &inst[i];
                char chapa_str[16];
                sprintf(chapa_str, "%dx%d", d->chapa_larg, d->chapa_alt);
                printf("| %-5s | %-10s | %-6d | %-10d | %-10d | %-8.2f | %-12.6f |\n", d->nome, chapa_str, d->cortes_nosso, d->area_chapa, d->area_utilizada, d->sobra_nossa, d->tempo_nosso);
