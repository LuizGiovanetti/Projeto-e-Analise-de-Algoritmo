...
Resultado construir_solucao_grasp(int chapa_larg, int chapa_alt, TipoCorte *tipos, int n_tipos, int *ordem_prioridade, double alfa, unsigned int *seed);
void busca_local_troca_prioridades(Instancia *d, int *prioridade, Resultado *melhor);
void busca_local_rotacao(Instancia *d, int *prioridade, Resultado *melhor);
...
void salvar_backups(Instancia *d);
void restaurar_backups(Instancia *d);
...
int main(int argc, char *argv[]){
    ...
    printf("=== SISTEMA DE CORTE DE CHAPA METALICA - GRASP ===\n");
    printf("Referencia: Chen & Huang (2007) - Tabela 1\n");
    printf("Parametros: Iteracoes=%d, Alfa=%.2f, Seed=%u\n", max_iteracoes, alfa, seed);
    ...
    for(inst = 0; inst < n_instancias; inst++){
        ...
        Resultado res = executar_grasp(d->chapa_larg, d->chapa_alt, d->tipos, d->n_tipos, d->prioridade, max_iteracoes, alfa, seed);
        clock_t fim = clock();
...
void salvar_backups(Instancia *d){
    d->tipos_backup = (TipoCorte*)malloc(sizeof(TipoCorte) * d->n_tipos);
    d->prioridade_backup = (int*)malloc(sizeof(int) * d->n_tipos);

    copiar_tipos(d->tipos_backup, d->tipos, d->n_tipos);
    copiar_prioridades(d->prioridade_backup, d->prioridade, d->n_tipos);
}

void restaurar_backups(Instancia *d){
    copiar_tipos(d->tipos, d->tipos_backup, d->n_tipos);
    copiar_prioridades(d->prioridade, d->prioridade_backup, d->n_tipos);

    free(d->tipos_backup);
    free(d->prioridade_backup);
}
...
void liberar_instancias(Instancia *inst, int n){
    for(int i = 0; i < n; i++){
        ...
        if(inst[i].tipos_backup) 
            free(inst[i].tipos_backup);

        if(inst[i].prioridade_backup) 
            free(inst[i].prioridade_backup);
    }
    free(inst);
}
...
Resultado construir_solucao_grasp(int chapa_larg, int chapa_alt, TipoCorte *tipos, int n_tipos, int *ordem_prioridade, double alfa, unsigned int *seed){
    ...
    while(n_cantos > 0){
        ...
        *seed = *seed * 1103515245 + 12345;
        ...
        int idx_escolhido = rcl_indices[escolha];
        int canto_escolhido = canto_indices[idx_escolhido];
        int tipo_escolhido = ordem_prioridade[tipo_indices[idx_escolhido]] - 1;

        Retangulo novo_corte;
        novo_corte.x = cantos[canto_escolhido].x;
        novo_corte.y = cantos[canto_escolhido].y;
        novo_corte.largura = tipos[tipo_escolhido].largura;
        novo_corte.altura = tipos[tipo_escolhido].altura;
        novo_corte.tipo = tipo_escolhido;

        res.posicoes[res.quantidade_cortes] = novo_corte;
        res.quantidade_cortes++;
        res.area_utilizada += novo_corte.largura * novo_corte.altura;
        res.contagem_por_tipo[tipo_escolhido]++;
...
Resultado executar_grasp(int chapa_larg, int chapa_alt, TipoCorte *tipos, int n_tipos, int *prioridade, int max_iteracoes, double alfa, unsigned int seed){
    ...
    for(iter = 0; iter < max_iteracoes; iter++){
        ...
        Instancia d_temp;
        d_temp.chapa_larg = chapa_larg;
        d_temp.chapa_alt = chapa_alt;
        d_temp.n_tipos = n_tipos;
        d_temp.tipos = tipos;
        d_temp.prioridade = prioridade;

        busca_local_rotacao(&d_temp, prioridade, &melhor);
        busca_local_troca_prioridades(&d_temp, prioridade, &melhor);
    }
...
void exibir_resultado_instancia(Instancia *d, Resultado res){
    ...
    for(i = 0; i < d->n_tipos; i++){
        ...
        for(p = 0; p < d->n_tipos; p++){
            if(d->prioridade[p] - 1 == i){ pos_pri = p + 1; break; }
        }
        char limite_str[20];
        if(d->tipos[i].max_quantidade > 0)
        ...
        printf("  Tipo %d (%dx%d cm, prior.%d, %s): %d peca(s)\n", i + 1, 
               d->tipos[i].largura, d->tipos[i].altura, pos_pri, limite_str, 
...
void exibir_tabela_comparativa(Instancia inst[], int n){
    int i;
    printf("\n============================================== INSTANCIAS C (COM REFERENCIA) ==============================================\n");
    printf("| %-5s | %-8s | %-6s | %-6s | %-9s | %-9s | %-8s| %-8s| %-12s| %-12s|\n", 
           "Inst.", "Chapa", "NCortesRef.", "NCortesNosso", "Area cm2", "Usada cm2", 
           "Sobra%Ref", "Sobra%Nos", "Tempo Ref(s)", "Tempo Nos(s)");
    printf("+-------+----------+-------------+--------------+-----------+-----------+----------+----------+-------------+-------------+\n");
    ...
    for(i = 0; i < n; i++){
        if(inst[i].nome[0] == 'C'){
            ...
            printf("| %-5s | %-8s | %-11d | %-12d | %-9d | %-9d | %-8.2f | %-8.2f | %-11.2f | %-11.6f |\n", 
                   d->nome, chapa_str, d->n_pecas_artigo, d->cortes_nosso, 
                   d->area_chapa, d->area_utilizada, d->sobra_artigo, d->sobra_nossa, 
                   d->tempo_artigo, d->tempo_nosso);
        }
    }
    printf("+-------+----------+-------------+--------------+-----------+-----------+----------+----------+-------------+-------------+\n");
    ...
    for(i = 0; i < n; i++){
        Instancia *d = &inst[i];
        if(d->n_pecas_artigo == 0) continue;
        tem = 1;
        printf("  %s -> Sobra: %s (%.2f%% nosso vs %.2f%% artigo) | Tempo: %s (%.6fs nosso vs %.2fs artigo)\n", 
               d->nome, d->sobra_nossa <= d->sobra_artigo ? "IGUAL/MELHOR" : "PIOR", 
               d->sobra_nossa, d->sobra_artigo, 
               d->tempo_nosso <= d->tempo_artigo ? "MAIS RAPIDO" : "MAIS LENTO", 
               d->tempo_nosso, d->tempo_artigo);
    }
    ...
    printf("\n========================== INSTANCIAS B (SEM REFERENCIA) ==========================\n");
    printf("| %-5s | %-10s | %-6s | %-10s | %-10s | %-8s | %-12s |\n", 
           "Inst.", "Chapa", "NCorte", "Area cm2", "Usada cm2", "Sobra%", "Tempo(s)");
    printf("+-------+------------+--------+------------+------------+----------+--------------+\n");
    ...
    for(i = 0; i < n; i++){
        if(inst[i].nome[0] == 'B'){
            ...
            char chapa_str[16];
            sprintf(chapa_str, "%dx%d", d->chapa_larg, d->chapa_alt);
            printf("| %-5s | %-10s | %-6d | %-10d | %-10d | %-8.2f | %-12.6f |\n", 
                   d->nome, chapa_str, d->cortes_nosso, d->area_chapa, 
                   d->area_utilizada, d->sobra_nossa, d->tempo_nosso);
        }
    }
    printf("+-------+------------+--------+------------+------------+----------+--------------+\n");
    printf("\n");
